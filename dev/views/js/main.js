...
// Using requestAnimationFrame to move the sliding
// background pizzas based on scroll position
...
// Only store latest known position on scroll 
// when browser chooses to execute
function onScroll() {
  latestKnownScrollY = window.scrollY;
  requestTick();
}

// updatePositions function improved with requestAnimationFrame and this
// tutorial: http://www.html5rocks.com/en/tutorials/speed/animations/

// Visual update applied only when user has scrolled
...
// Calculates size of screen to generate correct amount of "movers" 
// (pizzas) in background
var rows = screen.height/225;
var cols = screen.width/250;
var pizzaBgCalc = rows * cols;

function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");
  ticking = false;
  // Variables outside the for loop so that they are only loaded once
  var currentScrollY = latestKnownScrollY / 1250;
  var items = document.getElementsByClassName("mover");
  var phase= [];
  for (var i = 0; i < pizzaBgCalc; i++) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // declaring variable outside loop to prevent repetition in for loop
  for (var i = 0; i < pizzaBgCalc; i++) {
